...
/*      if (newX > 0 || newX < this.maxX) {
  ...
  ImageSlide.prototype.init = function(){
    ...
    _this.gallery = new SwipeView(_this.body.length > 0 ? _this.body[0] : _this.body, _.extend({numberOfPages : _this.slides.length}, _this.config));
    for(var i = 0 ; i < 3; i++){
      page = (i == 0 ? _this.slides.length - 1 : i - 1);
      _this.gallery.masterPages[i].innerHTML = '<a href = "'+_this.slides[page].href + (_this.config.boss ? '" boss="' + _this.config.boss : '') + '"><img crs="' + _this.slides[page].img + '" src="http://mat1.gtimg.com/auto/images/mobi/slide.png" style = "width:100%;"></a>';
    }

    $('img', _this.body).each(function(idx, el){
      var src = $(el).attr('crs');
      if(src){
        el.onload = function(e){
          $('.default', _this.body).remove();
          $(_this.nav).show();
        };
        el.onerror = function(e){
          $('.default', _this.body).remove();
          el.src = 'http://mat1.gtimg.com/auto/images/mobi/slide.png';
        }
        el.src = src;
      }
    });
    _this.gallery.onFlip(function(){
      ...
      for (i=0; i<3; i++) {
        upcoming = _this.gallery.masterPages[i].dataset.upcomingPageIndex;
        if (upcoming != _this.gallery.masterPages[i].dataset.pageIndex) {
          el = _this.gallery.masterPages[i].querySelector('img');
          el.src = _this.slides[upcoming].img;
        }
      }
      
      $('.'+selectedClass, _this.nav).removeClass(selectedClass);
      $($('li', _this.nav)[_this.gallery.pageIndex]).addClass(selectedClass);

    });

    _this.gallery.onTouchStart(function(){
      isswipeimg = '0';
    });

    _this.gallery.onTouchEnd(function(){
      isswipeimg = '1';
    });
  };

  /* 提供给客户端调用，决定是否禁止右滑关闭页面的操作 */
  window.canSwipeClose = function(){
    return !isswipeimg ? '1' : '0';
  }
